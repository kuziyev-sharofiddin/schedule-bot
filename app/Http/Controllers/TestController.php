    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->message)) {
            ...
            if (isset($request->message['text']) && $request->message['text'] == 'Yuborilgan postlar hisoboti') {
                ...
                foreach ($forwardedMessages as $forwardedMessage) {
                    $message = "Xabar " . "<b>" . $forwardedMessage['weekDay'] ."</b>". " kuni 🕦 "  . "<b>" .$forwardedMessage['time']. "</b>". " da yoboriladi.";
                    $this->telegram->sendMessageReply($forwardedMessage['chat_id'], $message, null, ['message_id' => $forwardedMessage['message_id']]);
                }
                return;
            }
...
class MessageController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (isset($request->message)) {
            ...
            // Start buyrug'i bilan boshlanishi
            if ($text == '/start') {
                cache()->put("step_$chat_id", 'start');
                $this->telegram->sendButtons($chat_id, "Assalomu Alaykum! Men yo'naltirilgan xabarlarni jadval asosida guruhga jo'natadigan botman. Quyidagilardan birini tanlang:", $this->buttons->report_detail_buttons);
                return;
            }

            // Yangi post joylash
            if ($text == 'Yangi post joylash') {
                cache()->put("step_$chat_id", 'new_post');
                $this->telegram->sendMessage($chat_id, "Iltimos postni guruhga yo'naltiring:", $this->buttons->back);
                return;
            }

            // Rejalashtirilgan postlar hisoboti
            if ($text == 'Rejalashtirilgan postlar hisoboti') {
                cache()->put("step_$chat_id", 'scheduled_posts');
                $this->telegram->sendButtons($chat_id, "Rejalashtirilgan postlar hisoboti:", $this->buttons->back);
                return;
            }

            // Yuborilgan postlar hisoboti
            if ($text == 'Yuborilgan postlar hisoboti') {
                cache()->put("step_$chat_id", 'sent_posts');
                $this->telegram->sendButtons($chat_id, "Yuborilgan postlar hisoboti:", $this->buttons->back);
                return;
            }
            ...
            if ($text == '⏪ Ortga') {
                if ($current_step == 'new_post' || $current_step == 'scheduled_posts' || $current_step == 'sent_posts') {
                    ...
                    $this->telegram->sendButtons($chat_id, "Asosiy menyuga qaytdingiz.", $this->buttons->report_detail_buttons);
                    return;
                } elseif ($current_step == 'time_selection') {
                    cache()->put("step_$chat_id", 'day_selection');
                    $report_buttons = $this->buttons->getReportButtons();
                    $this->telegram->sendButtons($chat_id, "Iltimos, hafta kunini belgilang:", $report_buttons);
                    return;
                } elseif ($current_step == 'day_selection') {
                    cache()->put("step_$chat_id", 'start');
                    $this->telegram->sendButtons($chat_id, "Asosiy menyuga qaytdingiz.", $this->buttons->report_detail_buttons);
                    return;
                }
            ...
            // Foydalanuvchi sana va vaqtni tanlayotganda
            if (isset($request->message['forward_from']) || isset($request->message['forward_from_chat'])) {
                cache()->put("step_$chat_id", 'day_selection');
                $report_buttons = $this->buttons->getReportButtons();
                $this->telegram->sendButtons($chat_id, "Iltimos, hafta kunini belgilang:", $report_buttons);
            ...
            // Kiritilgan kun va sana
            if ($this->dateFormatFunction->validateDateText($text)) {
                $today = Carbon::today();
                $threeDaysLater = Carbon::today()->addDays(3);
                $parts = explode("\n", $text);
                [$weekday, $date] = $parts;
                ...
                if ($date->greaterThanOrEqualTo($today) && $date->lessThanOrEqualTo($threeDaysLater)) {
                    cache()->put("selected_day_$chat_id", $weekday);
                    cache()->put("selected_date_$chat_id", $date->format('d.m.Y'));
                    cache()->put("step_$chat_id", 'time_selection');
                    $number_buttons = $this->buttons->getNumberButtons($weekday);
                    $this->telegram->sendButtons($chat_id, "Iltimos, vaqtni tanlang yoki qo'lda kiriting (24-soat formatida, masalan: 10:00):", $number_buttons);
                } else {
                    $report_buttons = $this->buttons->getReportButtons();
            ...
            // Vaqtni kiritish
            if ($current_step == 'time_selection') {
                $selected_date = cache()->get("selected_date_" . $chat_id);
                if ($selected_date) {
                    $checkerTime = $this->dateFormatFunction->validateDateTime($text, $selected_date);
                    if ($checkerTime === true) {
                        cache()->put("step_$chat_id", 'start');
                        $this->telegram->sendMessage($chat_id, "Vaqt to'g'ri kiritildi!");
                    } elseif ($checkerTime === false) {
                        $this->telegram->sendMessage($chat_id, "Iltimos, vaqtni to'g'ri kiriting. Vaqt hozirgi vaqtdan keyin bo'lishi kerak!");
                    } else {
