...
class CustomGoldFunctions
{
    protected $telegram, $buttons;

    public function __construct(Buttons $buttons)
    {
        $this->telegram = new Telegram(env('TELEGRAM_GOLD_TOKEN'));
        $this->buttons = $buttons;
    }

    public function chatBotTopic($chat_id, $request, $message, $is_working_time = null)
    {
        $message_thread_id = ChatBotTopic::query()->where('user_chat_id', $chat_id)->where('bot_name','Gold')->orderBy('id', 'DESC')->first();
                ...
                if ($is_working_time) {
                    ...
                    if ($message === "/start") { // /start messageni gruppaga yubormaslik (/startdan boshqa barcha msglar guruxga yuborilaveradi)
                        ...
                        if ($is_message_thread_id->object()->ok === false) { // Guruhdan client uchun ochilgan topic o'chirib yuborilgan xolat uchun
                            ...
                            if (!$old_topic) {
                                $topic = $this->telegram->createForumTopic($group_id, $request->message['from']['first_name'] . '-Gold');
    ...
    public function isEmptyAdmin($chat_id, $request, $is_working_time)
    {
        if ($is_working_time) {
            ...
            if ($admin !== null) {
                ...
                $old_topic = ChatBotTopic::query()->where('user_first_name', $request->message['from']['first_name'])->where('bot_name','Gold')->first();
                if (!$old_topic) {
                    $topic = $this->telegram->createForumTopic($group_id, $request->message['from']['first_name'] . '-Gold');
                }
                ChatBotTopic::create([
                    ...
                    'bot_name' => 'Gold', // har bir bot uchun alohida topic
                ]);
    ...
    public function getReserveClient($_is_admin,$bot_id)
    {
        ...
        if ($client_reserve !== null) {
            ...
            $old_topic = ChatBotTopic::query()->where('user_first_name', $client_reserve->user_first_name)->where('bot_name','Gold')->first();

            if (!$old_topic) {
                $topic = $this->telegram->createForumTopic($group_id, $client_reserve->user_first_name . '-Gold');
            }
            ChatBotTopic::create([
